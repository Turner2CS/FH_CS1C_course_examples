...
import week07_part03.HashEntries;
import week07_part03.HashEntry;
import week07_part03.utility.*;
...
public class FHmapSC<K,V>
{
    ...
    public boolean containsKey(K key) {
        ...
        // Check if the bucket in the table is empty.
        if (table[hashIndex] == null)
        {
            return false;
        }
        //  The bucket has one or more HashEntry objects, which have hashed in
        //  the hashIndex.
        ArrayList<HashEntry<K, V>> entriesInBucket = table[hashIndex].getDataEntries();
        //  Check whether we can match at least one HashEntry object with the
        //  caller's key of interest.
        for(HashEntry<K,V> element:entriesInBucket)
        {
            // Check if the current HashEntry object has the key of interest.
